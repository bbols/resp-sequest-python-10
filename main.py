...
print(f"2. Средняя зп от {sum([i for i in mas_salary_from if i is not None])/len(mas_salary_from)} до {sum([i for i in mas_salary_to if i is not None])/len(mas_salary_to) }")
...
for i,vac in enumerate(vacancies):
    temp_mas=filter_text(vac['snippet']['requirement'].replace('<highlighttext>','').replace('</highlighttext>',''))
